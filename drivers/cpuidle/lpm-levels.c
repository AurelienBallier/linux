		...
		struct lpm_cpu *cpu)
{
	...
	ktime_t delta_next;
	uint32_t sleep_us =
		(uint32_t)(ktime_to_us(tick_nohz_get_sleep_length(&delta_next)));
		...
		struct cpumask *mask, bool from_idle)
{
	...
	ktime_t next_event, tmp_next_event;
	...
	for_each_cpu(cpu, &online_cpus_in_cluster) {
		...
		tmp_next_event = hrtimer_get_next_event();
		if (tmp_next_event < next_event) {
			next_event = tmp_next_event;
			next_cpu = cpu;
		}
	}
