...
static int adv7533_read_device_rev(struct i2c_client *client)
{
	u8 rev = 0;
	int ret;

	ret = adv7533_read_byte(client->addr, ADV7533_REG_CHIP_REVISION, &rev);
	...
	struct adv7533_platform_data *pdata)
{
	...
	if (!(pdata->disable_gpios)) {
		...
		pdata->invert_sw = of_property_read_bool(np,
				"adv7533,invert-sw");

		pdata->cont_splash = of_property_read_bool(np,
	...
	bool on)
{
	...
	if (on) {
		...
		if (gpio_is_valid(pdata->switch_gpio)) {
			...
			ret = gpio_direction_output(pdata->switch_gpio, pdata->invert_sw?0:1);
			if (ret) {
				pr_err("unable to set dir for gpio [%d]\n",
					pdata->switch_gpio);
			...
			gpio_set_value(pdata->switch_gpio, pdata->invert_sw?0:1);
						 ...
						 const struct i2c_device_id *id)
{
	...
	ret = adv7533_read_device_rev(client_);
	if (ret != 0) {
	...
	if (!(pdata->disable_gpios)) {
		...
		if (pdata->adv_output) {
			gpio_set_value(pdata->switch_gpio, pdata->invert_sw?1:0);
		} else {
			gpio_set_value(pdata->switch_gpio, pdata->invert_sw?0:1);
			goto p_err;
		}
