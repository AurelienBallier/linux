		...
		struct kgsl_mem_entry *entry, struct kgsl_gpuobj_free *param)
{
	...
	ret = msm_copy_from_user(&event, to_user_ptr(param->priv),
		...
		struct kgsl_gpuobj_import *param)
{
	...
	ret = msm_copy_from_user(&useraddr,
		...
		int *fd)
{
	...
	ret = msm_copy_from_user(&buf, to_user_ptr(param->priv),
		...
		unsigned int cmd, void *data)
{
	...
	for (i = 0; i < param->count; i++) {
		ret = msm_copy_from_user(&objs[i], ptr, sizeof(*objs),
			param->obj_len);
		if (ret)
			goto out;
