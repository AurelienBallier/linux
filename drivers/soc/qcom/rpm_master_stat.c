		...
		struct msm_rpm_master_stats_private_data *prvdata)
{
	...
	if (prvdata->platform_data->version == 2) {
		...
		record.shutdown_req = readl_relaxed(prvdata->reg_base +
		...
		record.wakeup_ind = readl_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
		...
		record.bringup_req = readl_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
		...
		record.bringup_ack = readl_relaxed(prvdata->reg_base +
			(master_cnt * pdata->master_offset +
