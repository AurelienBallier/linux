...
struct cluster {
	...
	unsigned long mode_exit_timer_data;
	struct timer_list perf_cl_peak_mode_exit_timer;
	unsigned long perf_cl_peak_mode_exit_timer_data;
};
...
static int get_managed_cpus(char *buf, const struct kernel_param *kp)
{
	...
	for (i = 0; i < num_clusters; i++) {
		cnt += scnprintf(buf + cnt, PAGE_SIZE - cnt, 
											"%*pbl", cpumask_pr_args(managed_clusters[i]->cpus));
		if ((i + 1) >= num_clusters)
			break;
		cnt += snprintf(buf + cnt, PAGE_SIZE - cnt, ":");
	}
...
static void start_timer(struct cluster *cl)
{
	...
	if ((cl->mode & SINGLE) && !timer_pending(&cl->mode_exit_timer)) {
		...
		cl->mode_exit_timer_data = cpumask_first(cl->cpus);
...
static void start_perf_cl_peak_timer(struct cluster *cl)
{
	if ((cl->mode & PERF_CL_PEAK) &&
		...
		cl->perf_cl_peak_mode_exit_timer_data = cpumask_first(cl->cpus);
...
static void single_mod_exit_timer(struct timer_list *unused)
{
	int i;
	struct cluster *i_cl = NULL;
	unsigned long flags;
	...
	for (i = 0; i < num_clusters; i++) {
		if (cpumask_test_cpu(managed_clusters[i]->mode_exit_timer_data,
...
static void perf_cl_peak_mod_exit_timer(struct timer_list *unused)
{
	int i;
	struct cluster *i_cl = NULL;
	unsigned long flags;
	...
	for (i = 0; i < num_clusters; i++) {
		if (cpumask_test_cpu(managed_clusters[i]->perf_cl_peak_mode_exit_timer_data,
			managed_clusters[i]->cpus)) {
			i_cl = managed_clusters[i];
			break;
		}
...
static int init_cluster_control(void)
{
	...
	for (i = 0; i < num_clusters; i++) {
		...
		timer_setup(&managed_clusters[i]->mode_exit_timer, single_mod_exit_timer, 0);
		timer_setup(&managed_clusters[i]->perf_cl_peak_mode_exit_timer, perf_cl_peak_mod_exit_timer, 0);
